...
-export([usual_reduce/3,
         portion_reduce/4,
         portion_gen_reduce/4,
         limited_reduce/5,
         distributed_reduce/6,
         smartmsg_reduce/5,
         pool_reduce/6,
         pool_reduce_impl/5]).

-export([distributed_worker_adapter/2,
         pool_worker_adapter/5]).
...
distributed_reduce(Fun, SourceList, {InitValue, PortionInitValue}, PortionSize, NodeList, NodeWorkerCount) ->
    ReduceFun = fun(List) -> lists:foldl(Fun, InitValue, List) end,
    WorkerList = [spawn_link(Node, ?MODULE, distributed_worker_adapter, [Fun, PortionInitValue]) || Node <- NodeList, _WorkerIndex <- lists:seq(1, NodeWorkerCount)],
    Result = parallel_limited_helper:limited_core(ReduceFun, SourceList, PortionSize, WorkerList),
    lists:foldl(fun(Worker, _Aggr) -> exit(Worker, normal) end, true, WorkerList),
    Result.

distributed_worker_adapter(Fun, PortionInitValue) ->
    PortionReduceFun = fun(List) -> lists:foldl(Fun, PortionInitValue, List) end,
    parallel_limited_helper:limited_worker(PortionReduceFun).
...
pool_reduce_impl(Fun, SourceList, {InitValue, PortionInitValue}, PortionSize, WorkerCount) ->
    AssignFun = fun(Portion, Index, Master) -> pool:pspawn_link(?MODULE, pool_worker_adapter, [Fun, PortionInitValue, Portion, Index, Master]) end,
...
pool_worker_adapter(Fun, PortionInitValue, Portion, Index, Master) ->
    WorkerFun = fun() -> lists:foldl(Fun, PortionInitValue, Portion) end,
