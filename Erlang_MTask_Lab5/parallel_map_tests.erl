...
smartmsg_pmap_test_() ->
	[{"empty source", ?_assertEqual([], parallel_map:smartmsg_pmap(fun(Item) -> Item end, [], 2, 2))},
	 smartmsg_pmap_integer_list(),
	 smartmsg_pmap_string_list()].

smartmsg_pmap_integer_list() ->
	[{"integer list with small list size", ?_assertEqual([3, 6, 15], parallel_map:smartmsg_pmap(fun(Item) -> 3*Item end, [1, 2, 5], 4, 4))},
	 {"integer list with size equals portion size", ?_assertEqual([3, 6, 15], parallel_map:smartmsg_pmap(fun(Item) -> 3*Item end, [1, 2, 5], 3, 3))},
	 {"integer list with multiple portion", ?_assertEqual([3, 6, 15, 24, 21, 27], parallel_map:smartmsg_pmap(fun(Item) -> 3*Item end, [1, 2, 5, 8, 7, 9], 2, 2))},
	 {"integer list with nonmultiple portion", ?_assertEqual([3, 6, 15, 24, 21, 27, 12], parallel_map:smartmsg_pmap(fun(Item) -> 3*Item end, [1, 2, 5, 8, 7, 9, 4], 2, 2))},
	 {"integer list with one worker", ?_assertEqual([3, 6, 15, 24, 21, 27, 12], parallel_map:smartmsg_pmap(fun(Item) -> 3*Item end, [1, 2, 5, 8, 7, 9, 4], 2, 1))}].

smartmsg_pmap_string_list() ->
	[{"string list with small list size", ?_assertEqual(["31rts", "666rts", "99rts"], parallel_map:smartmsg_pmap(fun(Item) -> lists:reverse(Item) end, ["str13", "str666", "str99"], 4, 4))},
	 {"string list with size equals portion size", ?_assertEqual(["31rts", "666rts", "99rts"], parallel_map:smartmsg_pmap(fun(Item) -> lists:reverse(Item) end, ["str13", "str666", "str99"], 3, 3))},
	 {"string list with multiple portion", ?_assertEqual(["31rts", "666rts", "99rts", "7rts"], parallel_map:smartmsg_pmap(fun(Item) -> lists:reverse(Item) end, ["str13", "str666", "str99", "str7"], 2, 2))},
	 {"string list with nonmultiple portion", ?_assertEqual(["31rts", "666rts", "99rts"], parallel_map:smartmsg_pmap(fun(Item) -> lists:reverse(Item) end, ["str13", "str666", "str99"], 2, 2))},
	 {"string list with one worker", ?_assertEqual(["31rts", "666rts", "99rts"], parallel_map:smartmsg_pmap(fun(Item) -> lists:reverse(Item) end, ["str13", "str666", "str99"], 2, 1))}].