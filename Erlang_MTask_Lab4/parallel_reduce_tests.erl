...
smartmsg_reduce_test_() ->
	[{"empty source", ?_assertEqual(1, parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Item + Agg end, [], {1, 0}, 2, 2))},
	 smartmsg_reduce_integer_list(),
	 smartmsg_reduce_string_list()].

smartmsg_reduce_integer_list() ->
	[{"integer list with small list size", ?_assertEqual(4, parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Item + Agg end, [1, 2], {1, 0}, 4, 2))},
	 {"integer list with with size equals portion size", ?_assertEqual(7, parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Item + Agg end, [1, 2, 3], {1, 0}, 3, 2))},
	 {"integer list with with multiple portion", ?_assertEqual(11, parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Item + Agg end, [1, 2, 3, 4], {1, 0}, 2, 2))},
	 {"integer list with with nonmultiple portion", ?_assertEqual(7, parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Item + Agg end, [1, 2, 3], {1, 0}, 2, 2))},
	 {"integer list with with one worker", ?_assertEqual(7, parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Item + Agg end, [1, 2, 3], {1, 0}, 2, 1))}].

smartmsg_reduce_string_list() ->
	[{"string list with small list size", ?_assertEqual("++aabb", parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Agg ++ Item end, ["aa", "bb"], {"++", ""}, 4, 2))},
	 {"string list with with size equals portion size", ?_assertEqual("++aabbcc", parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Agg ++ Item end, ["aa", "bb", "cc"], {"++", ""}, 3, 2))},
	 {"string list with with multiple portion", ?_assertEqual("++aabbccdd", parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Agg ++ Item end, ["aa", "bb", "cc", "dd"], {"++", ""}, 2, 2))},
	 {"string list with with nonmultiple portion", ?_assertEqual("++aabbcc", parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Agg ++ Item end, ["aa", "bb", "cc"], {"++", ""}, 2, 2))},
	 {"string list with with one worker", ?_assertEqual("++aabbcc", parallel_reduce:smartmsg_reduce(fun(Item, Agg) -> Agg ++ Item end, ["aa", "bb", "cc"], {"++", ""}, 2, 1))}].